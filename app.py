...
def update_record(table_id, record_id, fields):
    ...
    # batch-update使用record_id_list+patch格式
    data = {"fields": fields}
...
def calculate_radar_data():
    ...
    for record_id, student in cache.students.items():
        ...
        # 归一化到0-100（假设每项满分100）
        homework = min(100, float(get_field_value(student, FLD_HOMEWORK) or 0))
        hygiene = min(100, float(get_field_value(student, FLD_HYGIENE) or 0))
        discipline = min(100, float(get_field_value(student, FLD_DISCIPLINE) or 0))
        sports = min(100, float(get_field_value(student, FLD_SPORTS) or 0))
        civil = min(100, float(get_field_value(student, FLD_CIVILITY) or 0))
        other = min(100, float(get_field_value(student, FLD_OTHER) or 0))
...
def calculate_warnings():
    ...
    for record_id, student in cache.students.items():
        ...
        student_records = [
            ...
            if name in parse_linked_records(get_field_value(r, FLD_STUDENT))
        ]
        ...
        # 预警1: 连续3天总积分为0
        recent_dates = sorted(daily_records.keys(), reverse=True)[:7]
        zero_days = 0
        ...
        for cat, count in category_violations.items():
            if count >= 3:
                warnings.append({
                    ...
                    "level": "🟠 橙色预警",
                    "student": name,
                    "reason": f"近21天{category_violations.get(cat, 0)}次{cat}类违规(扣分)",
                    "count": count
                })
...
def calculate_teacher_stats():
    ...
    # 分段统计
    ranges = {"0-100": 0, "100-200": 0, "200-300": 0, "300-400": 0, "400+": 0}
    for t in all_totals:
        if t < 100:
            ranges["0-100"] += 1
        elif t < 200:
...
def verify_login(username, password):
    ...
    if username.startswith("2026") and len(username) == 7:
        ...
        for record_id, student in cache.students.items():
            ...
            student_id = str(fields.get(FLD_STUDENT_ID, ""))

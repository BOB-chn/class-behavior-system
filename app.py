...
def mask_name(name):
    """姓名脱敏：两字隐藏第二字，三字隐藏第二字，四字隐藏第二三字"""
    if not name or name in ["未知", "教师", "值日班长1", "值日班长2", "值日班长3"]:
        return name
    n = len(name)
    if n == 2:
        return name[0] + "某"
    elif n == 3:
        return name[0] + "某" + name[2]
    elif n >= 4:
        return name[0] + "某" * (n - 2) + name[-1]
    return name
...
def parse_linked_records(val):
    ...
    飞书关联字段可能返回多种格式：
    1. [{record_id: "xxx", ...}] - 标准关联格式
    2. ["名字"] - 被get_field_value解包前的单元素数组（但get_field_value已自动解包）
    3. 纯文本 "名字" - 某些情况
    """
    if isinstance(val, list):
        ...
        for item in val:
            if isinstance(item, dict):
                ...
                # 可能是名字文本，尝试反查record_id
                rid = cache.students_by_name.get(item)
                if rid:
                    result.append(rid)
        return result
    elif isinstance(val, str):
        # 纯文本，可能是学生姓名
        rid = cache.students_by_name.get(val)
        if rid:
            return [rid]
    return []

def get_student_name_from_record(record):
    """从行为记录中提取学生姓名，兼容各种格式
    飞书关联字段可能的返回格式（经过get_field_value处理后）：
    1. {record_id: "xxx", text: "名字"} - 单元素数组被解包后的dict
    2. [{record_id: "xxx", ...}] - 未被解包的多元素数组
    3. "名字" - 纯文本（某些特殊情况）
    """
    val = get_field_value(record, FLD_STUDENT)
    
    # 情况1: get_field_value把单元素数组解包成了dict {record_id: "xxx", ...}
    if isinstance(val, dict):
        rid = val.get("record_id")
        if rid and rid in cache.record_id_to_name:
            return cache.record_id_to_name[rid]
        # 尝试从text字段获取
        text = val.get("text", "")
        if text:
            return text
    
    # 2. 如果val是列表（多元素数组未被解包）
    if isinstance(val, list):
        names = []
        for item in val:
            if isinstance(item, dict):
                rid = item.get("record_id")
                if rid and rid in cache.record_id_to_name:
                    names.append(cache.record_id_to_name[rid])
                else:
                    text = item.get("text", "")
                    if text:
                        names.append(text)
    ...
    # 3. 如果val被自动解包成了字符串（名字）
    if isinstance(val, str):
        if val in cache.students_by_name:
            return val
        # 可能是名字直接返回
...
def calculate_leaderboard():
    ...
    for record_id, student in cache.students.items():
        ...
        # 总积分：优先用公式字段，若公式字段为0但各维度有值则手动计算
        formula_total = float(get_field_value(student, FLD_TOTAL) or 0)
        calc_total = homework + hygiene + discipline + sports + civil + other
        total = formula_total if formula_total != 0 else calc_total
...
def calculate_warnings():
    ...
    for record_id, student in cache.students.items():
        ...
        # 获取学生总积分（公式字段兜底手动计算）
        hw = float(get_field_value(student, FLD_HOMEWORK) or 0)
        hy = float(get_field_value(student, FLD_HYGIENE) or 0)
        di = float(get_field_value(student, FLD_DISCIPLINE) or 0)
        sp = float(get_field_value(student, FLD_SPORTS) or 0)
        ci = float(get_field_value(student, FLD_CIVILITY) or 0)
        ot = float(get_field_value(student, FLD_OTHER) or 0)
        formula_total = float(get_field_value(student, FLD_TOTAL) or 0)
        total = formula_total if formula_total != 0 else (hw + hy + di + sp + ci + ot)
        ...
        student_records = [
            ...
            if get_student_name_from_record(r) == name
        ]
        ...
        # 只看有记录的最近7天，看是否连续3天有记录但零分或负分
        record_dates = sorted(daily_records.keys(), reverse=True)[:7]
        zero_score_days = 0
        for date in record_dates:
            day_total = sum(float(get_field_value(r, FLD_SCORE) or 0) for r in daily_records[date])
            if day_total <= 0:
                zero_score_days += 1
            else:
                break  # 一旦有正向积分就断开连续计数
        
        if zero_score_days >= 3:
            warnings.append({
                "type": "red",
                "level": "🔴 红色预警",
                "student": name,
                "reason": f"连续{zero_score_days}天有记录但无正向积分",
                "days": zero_score_days
            })
            continue
        
        # ========== 规则2: 连续3天作业扣分 ==========
        homework_negative_days = 0
        for date in record_dates:
            hw_records = [r for r in daily_records[date] 
                         if get_field_value(r, FLD_CATEGORY) == "作业"]
            if hw_records:
                has_negative = any(float(get_field_value(r, FLD_SCORE) or 0) < 0 for r in hw_records)
                if has_negative:
                    homework_negative_days += 1
                else:
                    break
            # 无作业记录的天跳过，不算连续
        
        if homework_negative_days >= 3:
            warnings.append({
                "type": "orange",
                "level": "🟠 橙色预警",
                "student": name,
                "reason": f"连续{homework_negative_days}天作业扣分",
                "days": homework_negative_days
            })
            continue
...
def calculate_trend_data():
    ...
    for record_id, student in cache.students.items():
        ...
        if not name:
            continue
        trend_data[name] = {d: 0 for d in dates}
    ...
    for r in cache.behavior_records:
        # 获取日期
        date = parse_timestamp(get_field_value(r, FLD_DATE))
        if not date or date not in date_set:
            continue
        
        score = float(get_field_value(r, FLD_SCORE) or 0)
        
        # 获取学生名 - 多种方式尝试
        student_name = get_student_name_from_record(r)
        
        if student_name and student_name in trend_data:
            trend_data[student_name][date] += score
    
    # 转换为列表格式
    result = {}
    for name, daily_scores in trend_data.items():
        cumulative = 0
        daily_list = []
        for date in dates:
...
def calculate_teacher_stats():
    ...
    # 积分分布
    # 积分分布（使用各维度手动计算的总分，避免公式字段读取失败）
    all_totals = []
    for s in cache.students.values():
        hw = float(get_field_value(s, FLD_HOMEWORK) or 0)
        hy = float(get_field_value(s, FLD_HYGIENE) or 0)
        di = float(get_field_value(s, FLD_DISCIPLINE) or 0)
        sp = float(get_field_value(s, FLD_SPORTS) or 0)
        ci = float(get_field_value(s, FLD_CIVILITY) or 0)
        ot = float(get_field_value(s, FLD_OTHER) or 0)
        formula_t = float(get_field_value(s, FLD_TOTAL) or 0)
        calc_t = hw + hy + di + sp + ci + ot
        all_totals.append(formula_t if formula_t != 0 else calc_t)
...
def student_records():
    """获取学生行为记录明细"""
    name = request.args.get("name", "")
    limit = int(request.args.get("limit", "5"))  # 默认5条，教师端可传limit=50
    if not name:
        return jsonify({"error": "缺少name参数"})
    ...
    for r in cache.behavior_records:
        student_name = get_student_name_from_record(r)
        if student_name == name or name in student_name:
            records.append({
                "date": parse_timestamp(get_field_value(r, FLD_DATE)),
                "category": get_field_value(r, FLD_CATEGORY),
...
def leaderboard():
    ...
    lb = []
    for item in cache.leaderboard:
        lb.append({**item, "masked_name": mask_name(item.get("name", ""))})
    ...
    for r in records:
        ...
        if status == "待审核":
            pending.append({
                ...
                "reporter": mask_name(get_field_value(r, FLD_REPORTER) or ""),
...
def approve_report():
    ...
    return jsonify({"success": True, "message": f"已通过 {mask_name(reporter)} 的 {category} 上报"})
...
@app.route("/api/monitor/handover-records")
def handover_records():
    """获取值日交接记录列表"""
    records = get_records(TBL_HANDOVER)
    result = []
    for r in records:
        result.append({
            "date": parse_timestamp(get_field_value(r, FLD_HANDOVER_DATE)),
            "monitor": get_field_value(r, FLD_DUTY_MONITOR) or "",
            "summary": get_field_value(r, FLD_HANDOVER_SUMMARY) or "",
            "pending": get_field_value(r, FLD_PENDING) or "",
            "note": get_field_value(r, FLD_HANDOVER_NOTE) or "",
            "status": get_field_value(r, FLD_HANDOVER_STATUS) or ""
        })
    # 按日期倒序
...
def teacher_dashboard():
    """教师看板"""
    masked_warnings = [{**w, "masked_student": mask_name(w.get("student", ""))} for w in cache.warnings]
    masked_lb10 = [{**item, "masked_name": mask_name(item.get("name", ""))} for item in cache.leaderboard[:10]]
    return jsonify({
        "stats": cache.teacher_stats,
        "warnings": masked_warnings,
        "leaderboard_top10": masked_lb10
    })
...
def get_warnings():
    """预警名单详情"""
    masked_warnings = [{**w, "masked_student": mask_name(w.get("student", ""))} for w in cache.warnings]
    return jsonify({"warnings": masked_warnings})
...
@app.route("/api/monitor/all-records")
def monitor_all_records():
    """值日班长获取行为记录（已审核通过的上报+值日班长录入的记录）"""
    # 1. 获取已审核的上报记录（已通过+已驳回）
    report_records = get_records(TBL_REPORTS)
    reviewed_reports = []
    for r in report_records:
        status = get_field_value(r, FLD_REVIEW_STATUS)
        if status in ["已通过", "已驳回"]:
            reviewed_reports.append({
                "date": parse_timestamp(get_field_value(r, FLD_REPORT_DATE)),
                "student": mask_name(get_field_value(r, FLD_REPORTER) or "未知"),
                "category": get_field_value(r, FLD_REPORT_CATEGORY),
                "rule": get_field_value(r, FLD_REPORT_DESC) or "",
                "score": float(get_field_value(r, FLD_REPORT_SCORE) or 0),
                "recorder": get_field_value(r, FLD_REVIEWER) or "学生上报",
                "status": status,
                "source": "上报"
            })
    
    # 2. 获取行为记录表中值日班长录入的记录（记录人包含"值日班长"的）
    behavior_records = []
    for r in cache.behavior_records:
        recorder = get_field_value(r, FLD_RECORDER) or ""
        student_name = get_student_name_from_record(r)
        if "值日班长" in recorder:
            behavior_records.append({
                "date": parse_timestamp(get_field_value(r, FLD_DATE)),
                "student": mask_name(student_name),
                "category": get_field_value(r, FLD_CATEGORY),
                "rule": get_field_value(r, FLD_RULE) or "",
                "score": float(get_field_value(r, FLD_SCORE) or 0),
                "recorder": recorder,
    ...
    # 合并并按日期倒序
    all_records = reviewed_reports + behavior_records
    all_records.sort(key=lambda x: x["date"] or "", reverse=True)
    
    return jsonify({"records": all_records[:100]})
...
def export_csv():
    ...
    for item in cache.leaderboard:
        ...
        lines.append(f"{student_id},{mask_name(item['name'])},{item['total']},{item['homework']},{item['hygiene']},{item['discipline']},{item['sports']},{item['civil']},{item['other']},{item['status']}")
